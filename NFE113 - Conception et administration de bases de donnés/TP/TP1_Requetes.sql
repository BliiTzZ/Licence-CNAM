 ...
 * Beneficiant d'une réduction
 * uniquement si le montant est inferieur à 200€
 * en tenant compte d'avoir des valeurs manquantes 
 */

SELECT ab.numeroAbonne, ab.nom, ab.prenom,ab.tarif*(1-ab.reduction / 100) AS montant
FROM Abonne ab
WHERE (ab.reduction IS NOT NULL OR ab.reduction <> 0)
AND tarif*(1-ab.reduction / 100) < 200
...
SELECT DISTINCT li.categorie
FROM Livre li
INNER JOIN Exemplaire ex
ON ex.idIsbn = li.isbn
WHERE ex.prix = (
    SELECT MAX(ex.prix)
    FROM Exemplaire ex
) 
...
FROM MotCle mc
EXCEPT
SELECT DISTINCT ca.mot+99
FROM Caracterisation ca
 ...
 */
 ...
 INNER JOIN Emprunt em
 ON em.numeroAbonne = ab.numeroAbonne
 WHERE em.nbRelance IS NOT NULL
 AND ab.age <  16
